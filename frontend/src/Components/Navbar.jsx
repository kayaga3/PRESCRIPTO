...
const Navbar = () => {
  ...
  return (
    <div className='flex items-center justify-between text-sm bg-white sticky top-0 py-4 mb-5 border-b border-b-gray'>
      ...
      <div className='flex items-center gap-4' >
        ...
        {/* mobile menu */}
        <div className={`${showMenu ? "-translate-x-0" : "translate-x-full"} fixed w-3/4 md:hidden right-0 top-0 bottom-0 z-20 overflow-hidden bg-primary bg-opacity-80 backdrop-blur-sm transform transition-transform duration-500 ease-in-out`}>
          <div className='flex items-center justify-between bg-slate-50 px-5 py-5'>
            <img className='w-36' src={assets.logo} alt="" />
            <img className='w-7' onClick={()=> setShowMenu(false)} src={assets.cross_icon} alt="" />
          </div>
          <ul className='flex flex-col items-center gap-2 mt-5 px-5 text-lg text-white font-medium'> 
